...
def compute_interval_from_inequalities(A, B):
    l, r = -np.inf, np.inf
...
def compute_Zu(beta_sh_info, a, b, w_tilde, n):
    ...
    return compute_interval_from_inequalities(psi, gamma)


def compute_Zv(beta_indiv_info, phi_u, iota_u, nK, a_tilde_u):
    nu0 = kappa0 = nu1 = kappa1 = np.empty(0)
    L, Lc, XK_L, XK_Lc, SL = beta_indiv_info["active_set"], beta_indiv_info["inactive_set"], beta_indiv_info["X_active"], beta_indiv_info["X_inactive"], beta_indiv_info["sign_active"]
    ...
    if len(L) > 0:
        ...
        kappa0 = (SL * ((XK_L_plus @ iota_u) - (inv @ (a_tilde_L * SL)))).ravel()
    ...
    if len(Lc) > 0:
        if len(L) == 0:
            ...
            proj = np.eye(nK) - XK_L @ XK_L_plus
            XK_L_plus_T = XK_L @ inv
            temp2 = (XK_Lc.T @ XK_L_plus_T) @ (a_tilde_L * SL)

        temp1 = XK_Lc.T @ proj

        term_b = (temp1 @ phi_u).ravel()
...
def compute_Zt(betaK_info, xi_uv, zeta_uv):
    ...
    return compute_interval_from_inequalities(omega, chi)


def calculate_phi_iota_xi_zeta(beta_sh_info, beta_indiv_info, XK, a, b, Q, rho, w_tilde, a_tilde, n, nK):
    p = XK.shape[1]
    phi_u = Q @ b
    iota_u = Q @ a
    xi_uv = np.zeros((p, 1))
    ...
    O, X_O, SO = beta_sh_info["active_set"], beta_sh_info["X_active"], beta_sh_info["sign_active"]
    L, XK_L, SL = beta_indiv_info["active_set"], beta_indiv_info["X_active"], beta_indiv_info["sign_active"]

    if len(O) > 0:
        w_tilde_O = w_tilde[O]
        Eu = np.eye(p)[:, O] 
        inv_X_O = pinv(X_O.T @ X_O)
        Eu_inv = (1 - rho) * (Eu @ inv_X_O)
        XK_Eu_inv = XK @ Eu_inv

        phi_u -=  XK_Eu_inv @ X_O.T @ b
        iota_u -= XK_Eu_inv @ (X_O.T @ a - w_tilde_O * SO)

        xi_uv += Eu_inv @ X_O.T @ b
        zeta_uv += Eu_inv @ (X_O.T @ a - w_tilde_O * SO)
    ...
    if len(L) > 0:
        ...
        zeta_uv += Fv_inv @ (XK_L.T @ iota_u - a_tilde_L * SL)

    return phi_u, iota_u, xi_uv, zeta_uv



def calculate_xi_zeta_star(beta_sh, beta_indiv_info, XK, phi, iota, rho, a_tilde):
    p = XK.shape[1]
    xi_uv = np.zeros_like(phi)
    zeta_uv = (1 - rho) * beta_sh.reshape(-1, 1)
    L, XK_L, SL = beta_indiv_info["active_set"], beta_indiv_info["X_active"], beta_indiv_info["sign_active"]
    ...
    if len(L) > 0:
        ...
        xi_uv += Fv_inv @ XK_L.T @ phi
        zeta_uv += Fv_inv @ (XK_L.T @ iota - a_tilde_L * SL)

    return xi_uv, zeta_uv



def compute_Zu_dtf(theta_info, c, d, q_tilde, n):
    psi0 = gamma0 = psi1 = gamma1 = np.empty(0)
...
def compute_Zv_dtf(delta_info, phi_u, iota_u, lambda_tilde, nK):
    nu0 = kappa0 = nu1 = kappa1 = np.empty(0)
